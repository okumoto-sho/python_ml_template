...
def main(_):
    ...
    velocity_model.load_state_dict(torch.load(FLAGS.checkpoint_path, weights_only=True))
    velocity_model.eval()
    scheduler = LinearScheduler()
    flow_matching = FlowMatching(model=velocity_model, scheduler=scheduler)

    with torch.no_grad():
        batch_size = 16
        x_0 = torch.randn(batch_size, 1, 32, 32).cuda()
